(function () {
  ...
  const dialog = createElement('div', {
    className: 'discord-dialog',
    attributes: {
      role: 'dialog',
      'aria-modal': 'true',
      'aria-labelledby': 'discord-dialog-title',
      'aria-hidden': 'true'
    }
  });

  const header = createElement('div', { className: 'discord-dialog__header' });
  ...
  const toggleDialog = (force) => {
    const isOpen = typeof force === 'boolean' ? force : !dialog.classList.contains('discord-dialog--active');
    dialog.classList.toggle('discord-dialog--active', isOpen);
    dialog.setAttribute('aria-hidden', isOpen ? 'false' : 'true');
  };

  closeButton.addEventListener('click', () => toggleDialog(false));
